        ...
        $scope.graphData = [
            {
                ...
                "Dependencies": [
                    ...
                    },
                    {
                        "ProductName": "ICS",
                        "Impact":
                            "When we add new column then whwn we search for Non person image in ICS then system should not throw error",
                        "DependentOn": "Patient Lookup"
                    },
                    {
                        "ProductName": "Patient Portal",
                        "Impact":
                            "If a new column is added to Patient Lookup, check In Patient Portal MailBox, while composing mail, in the To field, while searching for a patient, Patient Lookup should return only the patients with enrollment status as Pending or Completed ",
                        "DependentOn": "Patient Lookup"
                    },
                    {
                        "ProductName": "Patient Portal",
                        "Impact":
                            "If a new column is added to Patient Lookup, check In Patient Portal MailBox, When we open the Enrollment Request, if the patient is available in EPM then that patient should be automatically selected",
                        "DependentOn": "Patient Lookup"
                    }
                ]
            },
